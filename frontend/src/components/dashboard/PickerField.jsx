...
export function DateTimeField({
  ...
  const days = useMemo(() => buildCalendarDays(view.month, view.year), [view]);
  const yearOptions = useMemo(() => buildYearOptions(view.year), [view.year]);
  const minDateKey = useMemo(() => (disablePastDates ? todayDateKey() : ""), [disablePastDates]);
          ...
          <div className="picker-days">
            {days.map((dayValue, index) => (
              <button
                key={dayValue || `blank-${index}`}
                type="button"
                className={`picker-day ${dayValue === draft.date ? "selected" : ""} ${!dayValue ? "empty" : ""} ${dayValue && minDateKey && dayValue < minDateKey ? "past" : ""}`}
                onClick={() => {
                  if (!dayValue || (minDateKey && dayValue < minDateKey)) return;
                  setDraft({ ...draft, date: dayValue });
                }}
                disabled={!dayValue || Boolean(dayValue && minDateKey && dayValue < minDateKey)}
              >
                {dayValue ? Number(dayValue.slice(-2)) : ""}
              </button>
            ))}
