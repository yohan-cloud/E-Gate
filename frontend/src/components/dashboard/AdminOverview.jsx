      ...
      {loading ? (
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12, marginBottom: 16 }}>
            {cards.map((c) => (
              <div key={c.label} style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr", gap: 12 }}>
              ...
              {recentActivity.length === 0 && <div style={{ color: "#6b7280" }}>No recent activity.</div>}
              {recentActivity.map((item, idx) => (
                <div key={idx} style={{ padding: "8px 0", borderBottom: idx === recentActivity.length - 1 ? "none" : "1px solid #e5e7eb" }}>
                  <div style={{ fontWeight: 600, textTransform: "capitalize" }}>{item.type || "update"}</div>
                  <div style={{ color: "#475569", fontSize: 13 }}>
                    {item.username ? `${item.username} - ` : ""}
                    {item.title}
                  </div>
                  <div style={{ color: "#6b7280", fontSize: 12 }}>{item.timestamp ? new Date(item.timestamp).toLocaleString() : ""}</div>
                </div>
              ))}
              </div>
