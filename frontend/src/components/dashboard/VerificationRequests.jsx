  ...
  return (
    <div className="card" style={{ marginTop: 12 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12, marginBottom: 16 }}>
        {STATUSES.map((s) => (
          <div key={s.key} style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ color: "#475569", fontSize: 13 }}>{s.label}</div>
              <div style={{ fontSize: 28, fontWeight: 700 }}>{counts[s.key] ?? 0}</div>
            </div>
            <div style={{ width: 40, height: 40, borderRadius: 999, background: `${s.pillBg || "#e5e7eb"}`, display: "grid", placeItems: "center" }}>
      ...
      <div style={{ display: "inline-flex", gap: 8, marginBottom: 12, flexWrap: "wrap", padding: 4, background: "#f8fafc", borderRadius: 12 }}>
        {FILTERS.map((s) => (
          <button
            key={s.key}
      ...
      {loading ? (
        ...
        <div style={{ display: "grid", gap: 12 }}>
          {items.map((req) => (
            <div key={req.id} style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 16, display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 16, alignItems: "stretch" }}>
              ...
              <div style={{ display: "grid", justifyItems: "end", alignContent: "center", gap: 10, minWidth: 180 }}>
                {req.document_url && (
