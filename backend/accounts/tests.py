...
class PasswordResetEmailTests(TestCase):
    ...
    def test_request_password_reset_sends_email(self):
        res = self.client.post(
            ...
            {"username": "resident_reset", "email": "resident@example.com"},
            format="json",
        )
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_request_password_reset_rejects_wrong_email(self):
        res = self.client.post(
            "/api/accounts/password/otp/request/",
            {"username": "resident_reset", "email": "wrong@example.com"},
            format="json",
        )
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
