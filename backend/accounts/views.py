...
def login_admin(request):
    ...
    if not (getattr(user, "is_admin", False) or getattr(user, "is_gate_operator", False)):
        ...
        return Response({"error": "Access denied: Not an admin or gate operator account"}, status=status.HTTP_403_FORBIDDEN)
...
def request_password_reset_code(request):
    ...
    if stored_email.lower() != email.lower():
        return Response({"error": "Email address does not match our records."}, status=status.HTTP_400_BAD_REQUEST)
