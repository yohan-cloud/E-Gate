...
def gate_mark_attendance(request):
    return _mark_attendance_impl(request, None)
...
def gate_mark_attendance_face(request):
    return _mark_attendance_face_impl(request, None)
...
def export_event_attendance_csv(request, id):
    ...
    for a in rows:
        writer.writerow([
            ...
            a.checked_in_at,
            getattr(a.verified_by, 'username', ''),
        ])
    return response
...
class AttendanceAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        active_residents = ResidentProfile.objects.filter(expiry_date__gte=today_date).count()
        verified_ids = ResidentProfile.objects.filter(is_verified=True).count()
        ...
        for g_val, count in (
            ResidentProfile.objects.values_list("gender")
            .order_by()
        ...
        age_counts = {}
        residents = ResidentProfile.objects.exclude(birthdate__isnull=True)
